...
{
    //para no iterar tanto copiar y flipearlos  以免重复复制和翻转它们
    Objeto *objA = &A;
    Objeto *objB = &B;
    
    //dos comparaciones a la vez   一次进行两个比较
    for(int fig=0;fig<2;fig++)
    {
        ...
        for(int i=0;i<(int)vertA.size();i++)
        {
            ...
            //proyectamos todos los puntos en el axis y calculamos los limites   我们投影轴上的所有点并计算限制
            float a_min = INFINITY, a_max = -INFINITY;
            for(int a=0; a<(int)vertA.size();a++)
            {
            ...
            for(int b=0; b<(int)vertB.size();b++)
            {
                //producto punto nos da la escala del reflejo

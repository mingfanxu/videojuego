...
void Atlas::generar_mapa(SDL_Renderer *r,int idflip,int notidobjfisicos)
{
    ...
    for(int i=0;i<columna;++i)
    {
        ...
        for(int j=0;j<renglon;++j)
        {
            ...
            if(info.mapa_ids[i][j]==0 ||info.mapa_ids[i][j]==40 || info.mapa_ids[i][j]==1 || info.mapa_ids[i][j]==4
            || info.mapa_ids[i][j]==5 || info.mapa_ids[i][j]==45 || info.mapa_ids[i][j]==165 || info.mapa_ids[i][j]==205
            || info.mapa_ids[i][j]==204 || info.mapa_ids[i][j]==201 || info.mapa_ids[i][j]==200 || info.mapa_ids[i][j]==120
            || info.mapa_ids[i][j]==160)
            {
                tiene_colision=true;
            }

            //center not in colision

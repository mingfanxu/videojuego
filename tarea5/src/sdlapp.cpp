...
bool SDLApp::on_init()
{
    ...
    lista_personajes.push_back (new Personaje (100 , new Rectangulo (500,500,100,100,{0,0,0,255})));
...
void SDLApp::on_frameupdate ( double dt )
{
    ...
    for(auto &p:lista_personajes)
    {
        p->render(get().render);
        RenderTexto::get().render_texto(get().render,50,630,
            p->get_estadonombre(),120,30, SDL_Color {0,0,0,255});
    }
